...
def attack(message):
    ...
    if mob_hp <= 0:
        ...
        cursor.execute("UPDATE users SET exp=?, level=?, in_fight=0 WHERE user_id=?",
                       (exp, lvl, user_id))
    ...
    cursor.execute("UPDATE users SET hp=?, mob_hp=? WHERE user_id=?", (hp, mob_hp, user_id))
    conn.commit()
